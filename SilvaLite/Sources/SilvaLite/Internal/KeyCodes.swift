...
enum KeyPressParser {
    ...
    static func parse(_ specification: String) throws -> ParsedKeyPress {
        ...
        guard let keyToken = tokens.last else {
            throw SilvaLiteError.invalidKeySpec("Empty key specification")
        }

        let modifiers = try tokens.dropLast().map(parseModifier(_:))
        guard let keyCode = keyCodeMap[keyToken] else {
            throw SilvaLiteError.invalidKeySpec("Unknown key '\(specification)'")
        }

        return ParsedKeyPress(keyCode: keyCode, modifiers: modifiers)
    }
    ...
    private static func parseModifier(_ token: String) throws -> ParsedKeyPress.Modifier {
        ...
        case "cmd", "command", "super", "meta":
            return .init(flag: .maskCommand,   keyCode: CGKeyCode(kVK_Command))
        case "shift":
            return .init(flag: .maskShift,     keyCode: CGKeyCode(kVK_Shift))
        case "option", "alt":
            return .init(flag: .maskAlternate, keyCode: CGKeyCode(kVK_Option))
        case "control", "ctrl":
            return .init(flag: .maskControl,   keyCode: CGKeyCode(kVK_Control))
        default:
            throw SilvaLiteError.invalidKeySpec("Unknown modifier '\(token)'")
        }
    }
    ...
    private static let keyCodeMap: [String: CGKeyCode] = [
        ...
        "spacebar": CGKeyCode(kVK_Space),  "escape": CGKeyCode(kVK_Escape),
        "esc": CGKeyCode(kVK_Escape),      "backspace": CGKeyCode(kVK_Delete),
        "delete": CGKeyCode(kVK_Delete),   "del": CGKeyCode(kVK_ForwardDelete),
        ...
        "prior": CGKeyCode(kVK_PageUp),    "pagedown": CGKeyCode(kVK_PageDown),
        "page_down": CGKeyCode(kVK_PageDown), "next": CGKeyCode(kVK_PageDown),
